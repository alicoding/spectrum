var hljs = new function () {
  ...
  function m(r, u, p, lineNodes) {
    ...
    if (lineNodes) {
      ...
      resultPre.innerHTML = w.value;
      var linesPre = document.createElement('pre');
      var lines = escape(v).replace(/^/gm, '<span class="line"></span>');
      linesPre.innerHTML = lines;
      w.value = j(c(linesPre), c(resultPre), v);
    }
    ...
    function n() {
      ...
      Array.prototype.map.call(document.getElementsByTagName("pre"), b).filter(Boolean).forEach(function (o) {
        m(o, hljs.tabReplace, false, hljs.lineNodes)
      })
    }
